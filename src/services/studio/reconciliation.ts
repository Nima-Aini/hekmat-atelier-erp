import { db } from "@/db";
import { atelierExpenseSources, personnelSalaryRecords, rentalEquipment, studioContracts, studioProjectExpenses, studioProjectPayments, studioProjects } from "@/db/schema";
...
export async function getStudioReconciliationReport(allowedCoreProjectIds: string[] | null = null) {
  const [projectRows, contracts, payments, expenses, salaries, rentals, expenseSources] = await Promise.all([
    ...
    db.select().from(atelierExpenseSources),
  ]);
  ...
  const linkedSources = new Set(expenseSources.map((row) => `${row.sourceType}:${row.sourceId}`));
  ...
  for (const row of rentals) if (row.studioProjectId && !row.expenseId && !linkedSources.has(`rental:${row.id}`) && allowed(row.studioProjectId)) rows.push({ recordType: "studio_rental", studioRecordId: row.id, project: projectMap.get(row.studioProjectId)?.title || row.studioProjectId, amount: Number(row.rentalCost), date: row.pickupDate, currentLinkage: "expense: none", recommendedAction: "Verify supplier/account and manually reconcile to an ERP expense." });
