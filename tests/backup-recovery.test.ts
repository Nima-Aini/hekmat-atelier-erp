...
describe("native backup and restore safety", () => {
  ...
  it("verifies SHA-256 and rejects a corrupted archive", async () => {
    ...
    await db.insert(backups).values({ id, filename: `${id}.dump`, sizeBytes: archive.length, sizeBytesBigint: archive.length, checksum, status: "completed", storageDriver: "local", storageKey: id, format: "postgres_custom", formatVersion: 1, schemaVersion: "006_atelier_product" });
  ...
  it("reports safe deployment and schema identity in readiness", async () => {
    ...
    expect(response.status).toBe(200); expect(body).toMatchObject({ status: "ready", gitSha: "bb22e48c2299ddbefe6cee6854a21e6798a2e2df", schemaVersion: "006_atelier_product", environment: "staging" });
