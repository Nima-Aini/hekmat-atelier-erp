...
Live release QA caught a stylesheet mismatch despite successful SHA/readiness checks: the deployed HTML referenced CSS without the new overview selectors. Release build filesystem caching is disabled, and both `postbuild` and HTTP smoke now validate the CSS actually referenced by the homepage (not orphaned old/new files). This guard prevents a healthy API from being mistaken for a visually correct release. No database, financial operation or server credential is changed by this correction.
